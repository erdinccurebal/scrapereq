...
export async function controllerApiScrapeStart(req, res, next) {
  try {
    ...
    const { launchOptions, getProxy, pageAuthenticateEnabled, pageAuthenticateParams } =
      generateLaunchOptions({ res, proxies, accessPasswordWithoutProxy, proxyAuth });
...
function reqBodyValidation({ req, res }) {
  try {
    ...
    const { error, value } = helperValidatorsApiScrapeStart.validate(req.body, {
      abortEarly: false
    });
...
function generateLaunchOptions({ res, proxies, accessPasswordWithoutProxy, proxyAuth }) {
  ...
  try {
    ...
    const checkAccessPasswordWithoutProxy =
      accessPasswordWithoutProxy === process.env.ACCESS_PASSWORD_WITHOUT_PROXY;

    // Validate proxy requirements
    if (
      !checkAccessPasswordWithoutProxy &&
