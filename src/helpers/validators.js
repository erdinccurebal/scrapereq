...
export const helperValidatorsApiScrapeStart = Joi.object({
  ...
  selectors: Joi.array()
    ...
    .when('responseType', {
      ...
      then: Joi.forbidden().error(
        new Error('Selectors cannot be provided when responseType is NONE')
      ),
      otherwise: Joi.array()
        .items(
          Joi.object({
        ...
        .custom((selectors, helpers) => {
          ...
          const fullSelectors = selectors.filter(
            selector => selector.type === SELECTOR_TYPE_NAMES.FULL
          );
  ...
  steps: Joi.array()
    ...
    .custom((steps, helpers) => {
      ...
      const hasNavigateStep = steps.some(
        step => step.type === STEP_TYPES.NAVIGATE && (step.url || step.value)
      );
